...
fn main() {
    ...
    let serde_data = cgn::compress(cgn::serde_strategy::compress, &pgn_data);
    println!("Compressed data size: {}", serde_data.len());
    let serde_decompress_data = cgn::decompress(cgn::serde_strategy::decompress, &serde_data);
    assert_eq!(serde_decompress_data.to_string(), pgn_str);

    let serde_compress_data = cgn::compress(cgn::serde_compress_strategy::compress, &pgn_data);
    println!("Compressed data size: {}", serde_compress_data.len());
    let serde_compress_decompress_data = cgn::decompress(
